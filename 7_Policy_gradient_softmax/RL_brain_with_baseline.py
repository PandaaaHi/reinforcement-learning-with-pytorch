...
class PolicyGradient:
    def __init__(
        ...
        self.net_baseline = NetBaseline(self.n_features).to(self.device)

        self.loss = nn.CrossEntropyLoss(reduction='none')
        self.loss_baseline = nn.MSELoss()
        self.optimizer = optim.Adam(list(self.net.parameters()) + list(self.net_baseline.parameters()), lr=self.lr)
    ...
    def learn(self):
        ...
        baseline = self.net_baseline(ep_obs).squeeze(-1)

        # loss = self.my_loss(self.net(ep_obs), ep_as, ep_rs_norm)
        loss = self.my_loss(self.net(ep_obs), ep_as, (ep_rs_norm - baseline.detach())) + self.loss_baseline(baseline, ep_rs_norm) * 0.2
    ...
    def _discount_and_norm_rewards(self):
        ...
        mean = discounted_ep_rs.mean()
        std = discounted_ep_rs.std()
